...
for theft_occurrence in unique_theft_occurrences:
    ...
    for make_model in matched_make_models:
        ...
        if not top10_row.empty:
            ...
            output_df = output_df.append({"Theft_Occurrence": theft_occurrence, "Make_Model": make_model, "Thefts": thefts_value}, ignore_index=True)
    output_csv_filename = f'theft_occurrences_{theft_occurrence}.csv'
    output_df.to_csv(output_csv_filename, index=False)
    print(f'Results for theft occurrence {theft_occurrence} have been saved to {output_csv_filename}')
...
output_df.to_csv(output_csv_filename, index=False)
print(f'Mean Squared Error for Gradient Boosting Regressor has been saved to {output_csv_filename}')
...
output_df = pd.DataFrame({"Feature": sorted_features, "Feature Importance": sorted_importances})
